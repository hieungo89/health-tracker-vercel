...
const AccountCreation = ({ type }) => {
  ...
  const NameField = ({ label, text }) => (
    <label className="col-span-1" htmlFor={label}>
      *{text}:
      <input
        className="mx-1 px-2 bg-blue-100"
        type="text"
        name={label}
        placeholder={text}
        required
  ...
  const GoalField = ({ name, placeholder, required }) => (
    <li className="mx-4">
      <input
  ...
  const TextAreaField = ({ label, text }) => (
    <>
      <label htmlFor={label}>
        {text} (<em>optional</em>):
      </label>
