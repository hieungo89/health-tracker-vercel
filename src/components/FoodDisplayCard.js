...
const NutritionDisplayCard = ({ data, handleClick }) => {
  const minerals = [
    "Calcium",
    "Choline",
    "Copper",
  ...
  return (
    ...
    >
      <Card.Header css={{ placeContent: "center", padding: "0" }}>
        <h2 className="m-0 mt-4 text-h2 lg:text-h3">
          {format(dateConverted, "MM / dd / yyyy")}
        </h2>
        <button onClick={handleClick} className="absolute left-[91%] top-1">
          <CloseIcon />
        </button>
      </Card.Header>
      <Card.Body css={{ border: "solid", borderWidth: 1 }}>
        <h2 className="mb-0.5 md:text-h3">Nutrition Facts</h2>
        <Line length="short" />
        <Line length="short" />
        <h6 className="m-0">Total Daily Nutritioinal Value</h6>
        {/* //!Calories */}
        <h2 className="mb-0.5 flex justify-between md:text-h3">
          <span>Calories </span>
        ...
        <div className="flex justify-between pl-8 md:text-sm">
          ...
          <span>
            {data.totalNutrientCount["Saturated Fat"].perOfDailyNeeds.toFixed()}
            &#37;
          </span>
        </div>
        <Line length="medium" />
        ...
        <Line className="-mt-[85px] mb-0.5 " length="short" />
      ...
      <Card.Footer>
        <Text>
          ...
          {data.foodsEaten.length === 1 ? (
            <span>{data.foodsEaten[0]}.</span>
          ) : (
            <>
...
export { FoodDisplayCard, NutritionDisplayCard, SearchedFoodCards };
